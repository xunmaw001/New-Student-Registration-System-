<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.jiaowuchuName" type="text" placeholder="教务负责人名称"></input>
		</view>
...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view class="uni-product-title"
            ...
            {{product.jiaowuchuName}}</view>
        <view class="image-view">
            <image :style='{"borderRadius":"80rpx","width":"100%","height":"100%"}' mode="aspectFill"
                   class="uni-product-image" :src="product.jiaowuchuPhoto?product.jiaowuchuPhoto.split(',')[0]:''"></image>
        </view>
        <view class="uni-product-price" v-if="product.jiaowuchuNewMoney != null"
              :style='{"fontSize":"24rpx","lineHeight":"60rpx","color":"#fff","textAlign":"center"}'>
            <text class="">￥{{product.jiaowuchuNewMoney}}</text>
        </view>
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('jiaowuchu','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('jiaowuchu','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('jiaowuchu','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`jiaowuchu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiaowuchu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.jiaowuchuName) {
                    searchForm['jiaowuchuName'] = this.searchForm.jiaowuchuName
                }
				if (this.searchForm.jiaowuchuName) {
					searchForm['jiaowuchuName'] = this.searchForm.jiaowuchuName
				}

                let res = await this.$api.list(`jiaowuchu`, searchForm);
				// 如果是第一页数据置空
